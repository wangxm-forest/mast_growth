...
\item You need to check the DPI value for each image by going into the folder->30per, then check the metadata.json. You will see a line with dpi.
...
\item 
\item In the select measurement resolution section, make sure `Measure in milimetres' is selected, then change the new DPI value to the value you have earlier from the metadata.
...
\item When there are two samples for the same individual, finish the first one, save the measurement, and then click the CDendro icon in CooRecorder.
\item A window for the sample you just measured will pop up in CDendro, which you can use as a reference by clicking `select as reference'.
\item Repeat step 10-20 for the second sample, but this time, try to save the sample every 20 rings and then click the CDendro icon.
\item Make sure the offset is 0, if not, it means you missed a ring.
\item Make sure all the red numbers are above 0.4, if not, there might be some misidentified rings.
\item Record the CoorC in the excel sheet when finish all the measurement for these two cores.
