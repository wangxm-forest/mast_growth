...
\begin{align}
C_{i,t} &=\alpha + \alpha_{\text{site}} +\beta_{1} (\text{DBH}) + \beta_{2} (\text{climate variables})
\end{align}
where:
\begin{itemize}
    \item $\alpha$ is the intercept;
    \item $\alpha_{\text{site}}$ is the stand-specific intercept for site;
    \item $\beta_{1}$ is the slope for the relationship between tree size and carbon availability;
    \item $\beta_{2}$ is the slope for the relationship between climate variables and carbon availability.
\end{itemize}
...
\begin{align}
R_{i,t} &= \gamma \cdot sc \\
G_{i,t} &= \beta_{3} (rw)^{\beta_{4}} \cdot 0.5
\end{align}
where:
\begin{itemize}
    \item  $\gamma$ is the slope for relationship between seed count (sc) and carbon allocation to reproduction;
    \item $\beta_{3}$, $\beta_{4}$ are the parameters transforming ring width (rw) and height to carbon allocation to growth.
\end{itemize}
...
\begin{equation}
    sc_{i,t} \sim \text{NegBinomial} \left( \frac{R_{i,t}}{\gamma}, \phi_{sp} \right)
\end{equation}
...
\begin{equation}
    rw_{i,t} \sim \text{Lognormal} \left( \left[ \left( \frac{G_{i,t}}{0.5 \cdot \beta_{3}} \right)^{1/\beta_{4}} \right], \sigma^2_{rw} \right)
\end{equation}
