...
\begin{align}
C_{i,t} &=\alpha + \alpha_{\text{site}} +\beta_{dbh} (\text{DBH}) + \beta_{clim} (\text{climate variables})
\end{align}
where:
\begin{itemize}
    \item $\alpha$ is the intercept;
    \item $\alpha_{\text{site}}$ is the stand-specific intercept for site;
    \item $\beta_{dbh}$ is the slope for the relationship between tree size and carbon availability;
    \item $\beta_{clim}$ is the slope for the relationship between climate variables and carbon availability.
\end{itemize}
...
\begin{align}
R_{i,t} &= \gamma \cdot sc_{i,t} \\
G_{i,t} &= \beta_{growth,1} (rw_{i,t})^{\beta_{growth,2}} \cdot 0.5
\end{align}
where:
\begin{itemize}
    \item  $\gamma$ is the slope for relationship between seed count (sc) and carbon allocation to reproduction;
    \item $\beta_{growth,1}$, $\beta_{growth,2}$ are the parameters transforming ring width (rw) to carbon allocation to growth.
\end{itemize}
...
\begin{equation}
    sc_{i,t} \sim \text{NegBinomial} \left( \frac{R_{i,t}}{\gamma}, \phi \right)
\end{equation}
...
\begin{equation}
    rw_{i,t} \sim \text{Lognormal} \left( \left[ \left( \frac{G_{i,t}}{0.5 \cdot \beta_{growth}} \right)^{1/\beta_{height}} \right], \sigma^2 \right)
\end{equation}
where $\sigma^2$ represents the error variance.
